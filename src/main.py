...
class AddEmployeeWindow(QWidget):
    ...
    def init_ui2(self):
        ...
        self.password_input.setPlaceholderText("Enter PIN")
    ...
    def add_employee_to_db(self):
        ...
        # Validate password
        if not self.pinValidator(password):
            self.close()
            self.init_ui()

        # Check if name and password are provided
        if not name or not password:
            QMessageBox.warning(self, "Input Error", "Both name and PIN are required.")
            return
...
class MainAppWindow(QWidget):
    ...
    # Set background image only for the QScrollArea viewport, not for the employee labels
        # Set a fixed background image for the main window




        # Create the scroll area for employees (scrolls independently of background)
        self.scroll_area = QScrollArea()
        self.scroll_area.setWidgetResizable(True)
        self.scroll_area.setVerticalScrollBarPolicy(Qt.ScrollBarAlwaysOn)
        self.scroll_area.viewport().setObjectName("scrollViewport")
        # Set background image only for the QScrollArea viewport, not for the employee labels

        # Set a fixed background image for the scroll area viewport (not the labels)
